...
export function initDB() {
    ...
    `).run();
        ...
        try {
            const cols = db.prepare("PRAGMA table_info(hmi_pumps)").all();
            const hasPumpMode = cols.some((c) => c.name === 'pump_mode');
            if (hasPumpMode) {
        ...
        try {
            const tcols = db.prepare("PRAGMA table_info(hmi_tanks)").all();
            const hasBloco = tcols.some((c) => c.name === 'bloco');
            const hasCategoria = tcols.some((c) => c.name === 'categoria');
            const hasLocal = tcols.some((c) => c.name === 'local');
            if (!hasBloco)
                db.prepare("ALTER TABLE hmi_tanks ADD COLUMN bloco TEXT").run();
            if (!hasCategoria)
                db.prepare("ALTER TABLE hmi_tanks ADD COLUMN categoria TEXT CHECK(categoria IN ('inferior','superior'))").run();
            if (!hasLocal)
                db.prepare("ALTER TABLE hmi_tanks ADD COLUMN local TEXT").run();
        }
        catch { }
        // Migração: reforçar NOT NULL em readings (recriar se necessário)
        try {
            const rcols = db.prepare("PRAGMA table_info(readings)").all();
            const okTankId = rcols.some((c) => c.name === 'tank_id' && c.notnull === 1);
            const okLevel = rcols.some((c) => c.name === 'level' && c.notnull === 1);
            const okFlow = rcols.some((c) => c.name === 'flow' && c.notnull === 1);
...
export function loadStateFromDB() {
    const tanks = db.prepare("SELECT * FROM hmi_tanks ORDER BY id").all();
    const pumpsRaw = db.prepare("SELECT * FROM hmi_pumps ORDER BY id").all();
...
export async function DBUpdatePumpState(id, on, flow) {
    try {
        ...
        console.error(`ERRO AO SALVAR ESTADO NO DB: ${err.message}`);
    }
}
export async function DBUpdateTankLevel(id, level, flow) {
    try {
